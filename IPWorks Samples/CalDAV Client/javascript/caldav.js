...
async function main() {
    ...
    if (args.length < 4) {
        ...
        for (let i = 0; i < args.length; i++) {// args[i+1] corresponds to the value of argument [i]
            const arg = args[i];
            switch (arg) {
                case '-u':
                    user = args[i + 1];
    ...
    caldav1.setUser(user);
    if (password != null) {
        caldav1.setPassword(password)
    }
    ...
    rl.on('line', async line => {
        ...
        var cmd = argument[0];
        if (cmd == "1") {
            await caldav1.reset();// List Events and fill resource URIs, This must be done before deleting an event. 
            resourceURIs = [];
            eventNumber = 1;
            ...
            await caldav1.getCalendarReport("https://apidata.googleusercontent.com/caldav/v2/" + caldav1.getUser() + "/events").catch((e) => console.log(e.message));
            rl.prompt()
            ...
            await caldav1.reset();
            // Warning: After creating an event, it may take a few moments for the endpoint to be registered on the service side. 
            // Attempting to delete the event before this completes may result in a timeout or other unexpected behavior. 
            ...
            rl.question('Specify the number of the event to delete: ', async (index) => {
                ...
                await caldav1.deleteEvent(resourceURIs[index - 1]).catch(e => console.log(e.message))
...
function printUsage() {
    console.log("Usage:");
    console.log("  node caldav.js -u username -p password");
    console.log("Required arguments:");
    console.log("  -i    OAuth Client ID        Specify the client ID for OAuth authentication.");
    console.log("  -s    OAuth Client Secret    Specify the client secret associated with the client ID.");
    console.log("  -u    username               Your account username or email.");
    console.log("  -p    password               Your account password or app-specific password.");
    console.log("");
    console.log("Example:");
    console.log("  node caldav.js -u myemail@gmail.com -p mypassword -i <client_id> -s <client_secret>");
}
