...
begin
  ...
  if StatusCode <> 0 then
    lTrack.Items.Add(query + '->' + Description)
  else
  begin
    lTrack.Items.Add(IPInfo1.HostName + ' has address ' + IPInfo1.HostAddress);
    if IPInfo1.HostAliases <> '' then
      lTrack.Items.Add('alias ' + '. ' + IPInfo1.HostAliases);
    if IPInfo1.OtherAddresses <> '' then
      lTrack.Items.Add('other addresses ' +  '. ' + IPInfo1.OtherAddresses);
  end;
  Reqs.Delete(idx);
end;
...
begin
  ...
  for i := 1 to Length(tHost.Text) do
    if not (tHost.Text[i] in ['0'..'9','.']) then
      bAddr := FALSE;

  if bAddr then
    IPInfo1.HostAddress := tHost.Text
  else
    IPInfo1.HostName := tHost.Text;
...
begin
  ...
  label3.caption := IntToStr(IPInfo1.PendingRequests) + ' pending requests';
  tHost.Text := 'www.ibm.com';
  Button1Click(Sender);
  tHost.Text := 'www.nsoftware.com';
  Button1Click(Sender);
  tHost.Text := 'www.novell.com';
  Button1Click(Sender);
  tHost.Text := 'www.sun.com';
  Button1Click(Sender);
  tHost.Text := 'www.apple.com';
  Button1Click(Sender);
end;
