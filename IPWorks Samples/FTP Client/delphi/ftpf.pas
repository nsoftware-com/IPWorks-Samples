...
begin
  ...
  try
    ...
    if RadioGroupTransferMode.ItemIndex = 1 then
      ipwFTP1.ChangeTransferMode(Integer(tmBinary))
    else
    if RadioGroupTransferMode.ItemIndex = 2 then
...
begin
  ...
  if ListBoxRemFiles.Items.Strings[ListBoxRemFiles.ItemIndex][1] = '<' then
...
begin
  ...
  if ListBoxLocFiles.Items.Strings[ListBoxLocFiles.ItemIndex][1] = '<' then
