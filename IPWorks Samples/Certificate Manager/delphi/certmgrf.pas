...
type
  TFormCertmgr = class(TForm)
    ...
    procedure ipwCertMgr1CertList(Sender: TObject; CertEncoded: string; CertEncodedB: TBytes;
...
begin
  ...
  if pgCertsAndKeys.ActivePage = tabKeys then
  begin
...
procedure TFormCertmgr.ipwCertMgr1CertList(Sender: TObject; CertEncoded: string; CertEncodedB: TBytes;
  const CertSubject, CertIssuer, CertSerialNumber: String;
  HasPrivateKey: Boolean);
var
  listView: TListView;
...
begin
  if lvUserCerts.SelCount > 0 then
  begin
    ipwCertMgr1.CertStoreType := cstUser;
...
begin
  if lvMachineCerts.SelCount > 0 then
  begin
    ipwCertMgr1.CertStoreType := cstMachine;
...
begin
  if FormCreateCert = nil then
    FormCreateCert := TFormCreateCert.Create(FormCertmgr);
...
begin
  if FormSignCSR = nil then
    FormSignCSR := TFormSignCSR.Create(FormCertmgr);
  FormSignCSR.Show;
end;
...
begin
  ...
  if ipwCertMgr1.CertSubject = '' then
  begin
...
begin
  ...
  if MessageDlg('Are you sure you want to delete the certificate with subject: ' +
    ipwCertMgr1.CertSubject + '?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  begin
...
begin
  if (pgCertsAndKeys.ActivePage <> tabPFXStores) or (lvPFXCerts.Items.Count = 0)  or (lvPFXCerts.Selected = nil) then
  begin
